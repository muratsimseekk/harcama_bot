...
- "geçen ay kişisel ne kadar harcama yapmışım" → {{"ay": "geçen ay", "tip_filtre": "kisisel", "kategori_anahtar_kelimeler": [], "soru_ozet": "Geçen Ay - Tüm Kişisel Harcamalar"}}
...
def _harcamalari_filtrele(harcamalar: list[dict], analiz: dict) -> list[dict]:
    ...
    for h in harcamalar:
        tip_lower = h["tip"].lower().strip()
        if tip_filtre == "kisisel":
            if "isel" not in tip_lower and "şisel" not in tip_lower and "sisel" not in tip_lower:
                continue
        if tip_filtre == "isletme":
            if "letme" not in tip_lower:
                continue
...
def _rapor_png_olustur(harcamalar: list[dict], analiz: dict, ay_adi: str) -> str:
    ...
    if gun_toplam:
        gunler_sirali = sorted(
            gun_toplam.keys(),
            key=lambda x: datetime.strptime(x, "%d.%m.%Y") if (x != "—" and len(x) == 10) else datetime.min
        )
    ...
    # Ayırıcı çizgi — transform kullanmadan
    ...
    for i, h in enumerate(gosterilecek):
        renk = "#F1F5F9" if i % 2 == 0 else "#FFFFFF"
        ...
        for sx, deger in zip(sutun_x, degerler):
            renk_yazi = "#DC2626" if "letme" in deger.lower() else "#1E293B"
    ...
    fig.text(0.95, 0.03,
             ...
             ha="right", fontsize=13, fontweight="bold", color="white",
